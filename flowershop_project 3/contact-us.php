<?php
session_start();
$is_logged_in = isset($_SESSION['user_id']);
?>

<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Contact Us - Flower Shop</title>
    <link rel="stylesheet" href="assets/styles/home.css">
    <link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css" rel="stylesheet">
    <style>
        #form-message {
            color: white;
            padding: 10px 20px;
            border-radius: 5px;
            text-align: center;
            margin-bottom: 15px;
            box-shadow: 0 2px 5px rgba(0, 0, 0, 0.2);
            display: none; /* Initially hidden */
        }

        #form-message.success {
            background-color: #f0748b; /* Pink for success */
        }

        #form-message.error {
            background-color: #e55466; /* Red for errors */
        }

        header, footer {
            text-align: center;
            padding: 10px 0;
            background-color: #f8b1c4;
            color: white;
        }

        body {
            font-family: Arial, sans-serif;
            margin: 0;
            padding: 0;
            display: flex;
            flex-direction: column;
            min-height: 100vh;
            background-color: #f9f9f9;
        }

        main {
            flex: 1;
        }

        footer {
            background-color: #f8b1c4;
            color: #fff;
            text-align: center;
            padding: 10px;
        }
    </style>
</head>
<body>
    <header>
        <nav>
            <ul>
                <li><a href="<?php echo $is_logged_in ? 'home.php' : 'index.html'; ?>"><i class="fas fa-home"></i> Home</a></li>
                <li><a href="about-us.php"><i class="fas fa-info-circle"></i> About Us</a></li>
                <li><a href="contact-us.php"><i class="fas fa-envelope"></i> Contact Us</a></li>
                <?php if ($is_logged_in): ?>
                    <li><a href="profile.html"><i class="fas fa-user-edit"></i> Profile</a></li>
                    <li><a href="cart.php"><i class="fas fa-shopping-cart"></i> Cart</a></li>
                    <li><a href="logout.php"><i class="fas fa-sign-out-alt"></i> Logout</a></li>
                <?php else: ?>
                    <li><a href="sign-in.html"><i class="fas fa-sign-in-alt"></i> Sign In</a></li>
                    <li><a href="sign-up.html"><i class="fas fa-user-plus"></i> Sign Up</a></li>
                <?php endif; ?>
            </ul>
        </nav>
    </header>

    <main>
        <section class="page-content">
            <h2>Contact Us</h2>

            <!-- Message Box -->
            <?php if (isset($_GET['message'])): ?>
                <div id="form-message" class="<?php echo isset($_GET['status']) && $_GET['status'] === 'success' ? 'success' : 'error'; ?>" style="display: block;">
                    <?php echo htmlspecialchars($_GET['message']); ?>
                </div>
            <?php endif; ?>

            <form action="submit_contact.php" method="POST">
                <div class="form-group">
                    <label for="name"><i class="fas fa-user"></i> Name</label>
                    <input type="text" id="name" name="name" required placeholder="Enter your name">
                </div>

                <div class="form-group">
                    <label for="email"><i class="fas fa-envelope"></i> Email</label>
                    <input type="email" id="email" name="email" required placeholder="Enter your email">
                </div>

                <div class="form-group">
                    <label for="message"><i class="fas fa-comment"></i> Message</label>
                    <textarea id="message" name="message" rows="4" required placeholder="Enter your message"></textarea>
                </div>

                <button type="submit" class="btn-submit">Send Message</button>
            </form>

            <div class="contact-info">
                <h3>Contact Information</h3>
                <p><i class="fas fa-phone"></i> Phone: +20 128 456 5010</p>
                <p><i class="fas fa-map-marker-alt"></i> Address:  Lageteh St., Ibrahimia </p>
                <p><i class="fas fa-envelope"></i> Email: support@flowershop.com</p>
            </div>
        </section>
    </main>

    <footer>
        <p>&copy; 2024 Flower Shop. All rights reserved.</p>
    </footer>

    <script>
        // Hide the message after 10 seconds
        document.addEventListener("DOMContentLoaded", () => {
            const formMessage = document.getElementById("form-message");
            if (formMessage) {
                setTimeout(() => {
                    formMessage.style.transition = "opacity 1s";
                    formMessage.style.opacity = "0";
                    setTimeout(() => formMessage.style.display = "none", 1000);
                }, 10000); // 10 seconds
            }
        });
    </script>
</body>
</html>